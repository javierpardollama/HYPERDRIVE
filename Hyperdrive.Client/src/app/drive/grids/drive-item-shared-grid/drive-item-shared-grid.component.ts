...
export class DriveItemSharedGridComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  public dataSource: MatTableDataSource<ViewDriveItem> =
    new MatTableDataSource<ViewDriveItem>();
  ...
  public async FindPaginatedDriveItem(): Promise<void> {
    this.loading = true;
    const view =
      await this.driveItemService.FindPaginatedSharedDriveItemByApplicationUserId(
        this.page,
      );
    this.loading = false;
    ...
    if (view) {
      ...
      this.ELEMENT_DATA = Array.from(
        this.ELEMENT_DATA.concat(view.Items)
          .reduce(
            (m, t): Map<ViewDriveItem, ViewDriveItem> => m.set(t.Id, t),
            new Map(),
          )
          .values(),
      );
  ...
  public ApplyMyFilter(target: EventTarget | null): void {
    this.dataSource.filter = (target as HTMLInputElement).value
      .trim()
      .toLowerCase();
  }

  private TurnThePage = async (event: Event): Promise<void> => {
    let scroll: ViewScroll = new ViewScroll(
  ...
  public async GetLocalUser(): Promise<void> {
    this.User = await this.cryptoService.RetrieveObject<ViewApplicationUser>(
      VaultKeyAppVariants.VAULT_USER_KEY,
    );
  }

...
using Hyperdrive.Intelligence.Application.ViewModels.Removes;
...
namespace Hyperdrive.Intelligence.Service.Controllers.V1;
...
public class ChatController(IMediator @mediator) : ControllerBase
{
    ...
    public async Task<IActionResult> RemoveChat([FromBody] ViewRemoveChat @viewModel)
    {
    ...
    /// <response code="503">ServiceUnavailable</response>
    /// <response code="500">InternalServerError</response>     
    /// <param name="viewModel">Injected <see cref="ViewUpdateChatTitle"/></param>
    /// <returns>Instance of <see cref="Task{OkObjectResult{ViewChat}}"/></returns>   
    [MapToApiVersion(1.0)]
    [HttpPut]
    [Route("title/change")]
    ...
    [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(ProblemDetails))]
    [ProducesResponseType(StatusCodes.Status503ServiceUnavailable, Type = typeof(ProblemDetails))]
    public async Task<IActionResult> UpdateChatTitle([FromBody] ViewUpdateChatTitle @viewModel) => Ok(value: await mediator.Send(new UpdateChatTitleCommand { ViewModel = @viewModel }));


    /// <summary>
    /// Finds Paginated Chat By User Id
    /// </summary>
    /// <response code="200">Ok</response>
    /// <response code="400">BadRequest</response>
    /// <response code="401">Unauthorized</response>
    ...
    /// <response code="503">ServiceUnavailable</response>
    /// <response code="500">InternalServerError</response>     
    /// <param name="viewModel">Injected <see cref="FilterPageChat"/></param>
    /// <returns>Instance of <see cref="Task{OkObjectResult{ViewPage{ViewChat}}"/></returns>   
    [MapToApiVersion(1.0)]
    [HttpPost]
    [Route("page")]
    ...
    [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(ProblemDetails))]
    [ProducesResponseType(StatusCodes.Status503ServiceUnavailable, Type = typeof(ProblemDetails))]
    public async Task<IActionResult> FindPaginatedChatByUserId([FromBody] FilterPageChat @viewModel) => Ok(value: await mediator.Send(new FindPaginatedChatQuery { ViewModel = @viewModel }));
}
